...
class OraFileFormatIntegrationTest {
    ...
    fun testSaveAsOraFile() {
        onDrawingSurfaceView().perform(touchAt(com.widgetly.iartbook.test.espresso.util.DrawingSurfaceLocationProvider.MIDDLE))
    ...
    fun testSaveAndOverrideOraFile() {
        ...
        onDrawingSurfaceView().perform(touchAt(com.widgetly.iartbook.test.espresso.util.DrawingSurfaceLocationProvider.BOTTOM_MIDDLE))
        onTopBarView().performOpenMoreOptions()
        onView(withText(R.string.menu_save_image)).perform(ViewActions.click())
        onView(withText(R.string.save_button_text)).perform(ViewActions.click())
    ...
    fun testOraFileWithMultipleLayersSaveAndLoad() {
        ...
        com.widgetly.iartbook.test.espresso.util.wrappers.LayerMenuViewInteraction.onLayerMenuView()
            ...
            .checkLayerCount(2)
            .performClose()
        onDrawingSurfaceView().perform(touchAt(com.widgetly.iartbook.test.espresso.util.DrawingSurfaceLocationProvider.HALFWAY_TOP_MIDDLE))
        com.widgetly.iartbook.test.espresso.util.wrappers.LayerMenuViewInteraction.onLayerMenuView()
            .performOpen()
            .performAddLayer()
            .checkLayerCount(3)
            .performClose()
        onDrawingSurfaceView().perform(touchAt(com.widgetly.iartbook.test.espresso.util.DrawingSurfaceLocationProvider.HALFWAY_BOTTOM_MIDDLE))
        onTopBarView().performOpenMoreOptions()
        onView(withText(R.string.menu_save_image)).perform(ViewActions.click())
        onView(withId(R.id.pocketpaint_save_dialog_spinner)).perform(ViewActions.click())
        onData(AllOf.allOf(`is`(instanceOf<Any>(String::class.java)), `is`<String>("ora")))
