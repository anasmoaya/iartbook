 ...
 */
package com.widgetly.iartbook.test.espresso.util
...
object EspressoUtils {
    ...
    fun waitForToast(viewMatcher: Matcher<View?>?, duration: Int) {
        ...
        val viewInteraction = Espresso.onView(viewMatcher).inRoot(com.widgetly.iartbook.test.espresso.util.UiMatcher.isToast())
        while (System.currentTimeMillis() < waitTime) {
            try {
                viewInteraction.check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
                return
            } catch (e: NoMatchingViewException) {
                Espresso.onView(ViewMatchers.isRoot()).perform(com.widgetly.iartbook.test.espresso.util.UiInteractions.waitFor(250))
            }
        }
        viewInteraction.check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
    }
