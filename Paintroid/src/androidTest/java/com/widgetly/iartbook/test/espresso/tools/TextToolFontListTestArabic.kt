...
class TextToolFontListTestArabic {
    ...
    fun testTextFontFaceOfFontSpinnerArabic() {
        ...
        Assert.assertTrue(com.widgetly.iartbook.test.espresso.rtl.util.RtlUiTestUtils.checkTextDirection(Locale.getDefault().displayName))
        onToolBarView()
            .performSelectTool(ToolType.TEXT)
        onView(ViewMatchers.withId(R.id.pocketpaint_text_tool_dialog_list_font))
            .check(
                ViewAssertions.matches(
                    com.widgetly.iartbook.test.espresso.util.UiMatcher.atPosition(
                        0,
                        ViewMatchers.hasDescendant(
                            com.widgetly.iartbook.test.espresso.util.UiMatcher.hasTypeFace(
                                sansSerifFontFace
                            )
                        )
                    )
            ...
            .check(
                ViewAssertions.matches(
                    com.widgetly.iartbook.test.espresso.util.UiMatcher.atPosition(
                        4,
                        ViewMatchers.hasDescendant(
                            com.widgetly.iartbook.test.espresso.util.UiMatcher.hasTypeFace(stcFontFace)
                        )
                    )
                )
            )
